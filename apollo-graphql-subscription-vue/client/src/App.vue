<template>
  <div id="app">
    ...
    <div class="tag-of-the-day" v-if="randomTag">
      ...
      <div class="tag">{{ randomTag.label }} - {{ randomTag.type }}</div>
    </div>
    <h2>Tags</h2>
    <div class="info">
      <label>
        <input type="checkbox" v-model="skipQuery">Skip query (this disables the apollo query)
      </label>
    </div>
    <div class="info">Query updates: {{ updateCount }}</div>
    <div class="type">
      <label>
    ...
    <div class="tag-list">
      <template v-if="tagsPage">
        <div
          class="tag-list-item"
          v-for="tag in tagsPage.tags"
        >{{ tag.id }} - {{ tag.label }} - {{ tag.type }}</div>
      </template>
        ...
        .mutate({
          ...
          updateQueries: {
            tagList: (previousResult, { mutationResult }) => {
              ...
              if (
                previousResult.tags.find(
                  tag => tag.id === mutationResult.data.addTag.id
                )
              ) {
                return previousResult;
              }
